...
__all__ = [
    "__version__",
    "RCWASolver", "FDFDSolver2D", "FDTDSolver2D", "FDTDSolver3D",
    "NeuralSurrogate",
...
def __getattr__(name):
    if name in ("RCWASolver", "FDFDSolver2D", "FDTDSolver2D", "FDTDSolver3D",
                "NeuralSurrogate", "HopkinsLithoModel", "LearnedFabModel",
                "DifferentiableResistModel"):
        from diffnano import solvers
        return getattr(solvers, name)
    if name in ("MetalensDesigner", "DFMMetalensDesigner", "PhCDesigner",
                "WaveguideDesigner", "BroadbandOptimizer", "MultiObjectiveExplorer",
                "EndToEndPipeline"):
